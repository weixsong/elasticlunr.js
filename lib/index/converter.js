...
import deprom from 'synchronized-promise';
import { QueryRepository, MatchQuery, BoolQuery } from '../dsl.js';
...
export default class Converter {
    ...
    saveDocument(toSave) {
        for (var o of Object.values(this.inner._fields)) {
            o._store = toSave;
        }
    }
    ...
    setRef(field) {
        return deprom(this.inner.setRef(field));
    }
    ...
    elasticsearch(query) {
        ...
        // console.log(util.inspect(q.rewrite(this.inner), false, null, true));
        ...
        if (typeof query == "string") {
            ...
            if (!options || typeof options !== "object") {
                return this.elasticsearch({
                    "query": {
                        "bool": {
                            "should": this.inner.getFields().filter((field) => field != this.inner.getRef()).map( (field) => { return {
                                "match": {
                                    [field]: query
                                }
                            }})
                ...
                return new BoolQuery({
                    should: this.inner.getFields().filter((field) => field != this.inner.getRef()).map( (field) => new MatchQuery({
                        field: field,
                        query: query
                    }))
                }).score(this.inner).sort((a, b) => a.score < b.score);
            }
            ...
            if (options.fields) {
                return this.elasticsearch({
                    "query": {
                        "bool": {
                            "should": this.inner.getFields().filter((field) => field !== this.inner.getRef() && (options.fields[field] && options.fields[field].boost > 0)).map( (field) => { return {
                                match: {
                                    [field]: query
                                },
                ...
                return new BoolQuery({
                    should: this.inner.getFields().filter((field) => field !== this.inner.getRef() && (options.fields[field] && options.fields[field].boost > 0)).map( (field) => new MatchQuery({
                        field: field,
                        query: query,
                        boost: options.fields[field].boost
                    }))
                    ...
                    "bool": {
                        "should": search.map( ([field, content]) => {
                            return {
                                ...
                                "expand": options && options.expand ? true : false,
